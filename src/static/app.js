    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        activityCard.addEventListener("click", async (event) => {
          ...
          const activityToRemove = decodeURIComponent(removeBtn.dataset.activity);
          const emailToRemove = decodeURIComponent(removeBtn.dataset.email);

          try {
            const resp = await fetch(
              `/activities/${encodeURIComponent(activityToRemove)}/participants?email=${encodeURIComponent(
                emailToRemove
              )}`,
              { method: "DELETE" }
            );
